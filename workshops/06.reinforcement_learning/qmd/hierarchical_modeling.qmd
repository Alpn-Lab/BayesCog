...
<!-- TODO (reviewer R2.3): add prior distribution images for Normal(0,1) and a Beta distribution here, and note the difference between using a truncated Normal vs a (scaled) Beta for bounded parameters. -->
...
It is worth noting that this "wall" picture describes a *direct* implementation of MCMC. Modern samplers such as the No-U-Turn Sampler (NUTS) used by Stan do not actually sample bounded parameters directly: they internally *transform* a bounded parameter onto an unconstrained space (e.g. via a log or logit transformation), sample there - where there are no walls to hit - and then transform back. As a result, the practical difficulties these samplers encounter usually arise not from the boundaries themselves, but from the *geometry* of the posterior, such as funnels or sharp ridges (something we return to in the next workshop on reparameterization).
:::

:::{.callout-important title="Setting priors"}
The priors used in this tutorial are chosen for pedagogical reasons and may not be appropriate for your particular research question. For example, a beta distribution prior may be preferred for the X parameter instead of the Normal(0, 1). You should set your priors according to several factors including your specific research context, theoretical understanding, and existing literature in your field.
:::

(TO DO: add prior distribution images for Normal(0,1) and beta distribution, and note the difference)
