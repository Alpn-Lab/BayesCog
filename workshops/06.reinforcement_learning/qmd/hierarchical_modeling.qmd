...
$P(\Phi)$ the population level: our prior knowledge about the population-level parameters (e.g., group mean and variance)
...
It is worth noting that this "wall" picture describes a *direct* implementation of MCMC. Modern samplers such as the No-U-Turn Sampler (NUTS) used by Stan do not actually sample bounded parameters directly: they internally *transform* a bounded parameter onto an unconstrained space (e.g. via a log or logit transformation), sample there - where there are no walls to hit - and then transform back. As a result, the practical difficulties these samplers encounter usually arise not from the boundaries themselves, but from the *geometry* of the posterior, such as funnels or sharp ridges (a topic covered in the next workshop on reparameterization).

:::{.callout-important title="Setting appropriate priors"}
The priors used in this tutorial are chosen for pedagogical reasons and **may not be appropriate for your particular research question.** Priors should be informed by several factors, including your specific research context, your theoretical understanding, and the existing literature in your field.

For example, the learning rate $\alpha$ is bounded between 0 and 1, and here we place a truncated Normal prior on it (illustrated on the left below). But a **Beta distribution** - which is naturally defined on the interval $[0,1]$ - is a natural alternative for such a bounded parameter, and avoids the need for truncation altogether.

![](/images/06.reinforcement_learning/hrch/prior_normal_vs_beta.png){fig-align="center" width="90%"}

::: {.image-caption style="font-size: 1.5em;"}
Two candidate priors for the learning rate: a truncated Normal and a Beta(2, 5), both defined on the interval [0, 1]
:::

We could in fact have used the $Beta(2, 5)$ shown on the right as our prior for the learning rate. Unlike the truncated Normal, it lives on $[0,1]$ naturally, and its rightward skew places more weight on lower learning rates - encoding an assumption that participants tend to learn relatively slowly. This would be a sensible choice if prior evidence or the existing literature suggested that learning rates are typically low.
:::
