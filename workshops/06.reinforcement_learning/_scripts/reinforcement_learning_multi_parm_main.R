...
run_rl_mp <- function(modelType = 'indv') {
    ...
    # individually fitting but not hierarchical
    if (modelType=='indv') {
        ...
        # added by A.S. to plot mean group-level parameter densities
        plot_dens <- stan_plot(fit_rl, pars=c('lr_mu','tau_mu'), show_density=T, fill_color = 'skyblue')
