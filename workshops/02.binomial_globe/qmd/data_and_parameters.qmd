...
subtitle: "Understanding how Bayesian inference allows us to update our knowledge with data"
execute:
  echo: true
  warning: false
  message: false
...
And finally, we learned about Bayes' theorem, a formula which allows us to update our knowledge based on new evidence in a probabilistic manner:

$$p(A|B) = \frac{p(B|A)p(A)}{p(B)}$$

Where:

- $p(A|B)$ is the **posterior probability** - our updated knowledge about $A$ after observing evidence $B$
- $p(B|A)$ is the **likelihood** - probability of observing evidence $B$ if hypothesis $A$ is true
- $p(A)$ is the **prior probability** - our initial knowledge about $A$ before seeing any evidence
...
Remember the purpose of Bayesian analysis concerns updating our initial knowledge in light of data.

<center>
$\Large\text{initial knowledge}$ $\Large\xrightarrow{\text{Bayes' rule + data}}$ $\Large\text{updated knowledge}$
</center>

<div style="height: 20px;"></div>

Therefore, we need some initial knowledge to begin with!

A 'prior' - $p(\theta)$ - represents our assumptions about parameters before seeing any data. **These assumptions can come from previous research, expert knowledge, or logical constraints about what values are possible.**
...
Alternatively, we could use the proportion of the UK population that has this disease as a prior. In this case, the prior is continuous and represents our prior knowledge of the prevalence.
