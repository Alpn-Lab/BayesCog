...
While trace plots remain a common and intuitive diagnostic, they can hide subtle differences between chains. *Rank plots* are often more informative: they pool all draws across chains, rank them, and then show the distribution of ranks within each chain. If the chains are sampling from the same distribution, these should look approximately uniform. The `bayesplot` package provides these, for example:
...
<!-- TODO (reviewer R2.7): add actual rank plots generated from our globe-tossing model parameters (run the code above, save the figure to images/, and embed it here). -->
:::

(TO DO: add actual rank plots from our parameters)
