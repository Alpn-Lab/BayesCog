...
However, there is an additional line, which is optional in this specific case. Recall that in Bayesian inference, we need a prior (initial assumption) for our parameters. In this case, we could (but don't have to), set a uniform prior between 0 and 1.
...
- **Effective Sample Size (n_eff):** This is an estimate of the number of *independent* samples that would carry the same amount of information as the (autocorrelated) samples we actually got from our model. Because successive MCMC draws are correlated, our 4000 total samples are not all independent of one another. The `n_eff` of 1483 means our chains carry roughly as much information as 1,483 independent draws, which is a good value; a general rule of thumb is that we want this to be above 1000.

- **R-hat (Rhat, $\hat{R}$):** $\hat{R}$ helps us check whether our chains are converging to the same distribution. 1 is the ideal value, and values above 1.1 are problematic [^4].
...
[^4]: Vehtari, A., Gelman, A., Simpson, D., Carpenter, B., & Bürkner, P.-C. (2021). Rank-normalization, folding, and localization: An improved $\hat{R}$ for assessing convergence of MCMC. Bayesian Analysis, 16(2), 667-718.
...
:::{.callout-tip title="Rank plots: a more sensitive alternative to trace plots"}
While trace plots remain a common and intuitive diagnostic, they can hide subtle differences between chains. *Rank plots* are often more informative: they pool all draws across chains, rank them, and then show the distribution of ranks within each chain. If the chains are sampling from the same distribution, they should look approximately uniform. The `bayesplot` package provides these, for example:

```r
library(bayesplot)

# use the same chain colours as stan_trace() so the two plots match
...
![](/images/02.binomial_globe/stan/mcmc_rank_ecdf.png){fig-align="center" width="85%"}

The plot above shows the result for our globe-tossing model. The idea behind it is simple: if all four chains are exploring the posterior in the same way, then no chain should consistently produce higher or lower values than the others. Here, all four lines wobble around zero and stay inside the envelope, so no chain stands out as behaving differently. This confirms what the trace plots already suggested: the chains have converged and are sampling from the same distribution.
:::
