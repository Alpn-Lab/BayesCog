...
Various analyses across the workshops require specific packages to be installed. This project uses `renv` to manage package dependencies. 

To set up the environment:

1. Open `R` in the project directory (`BayesCog.Rproj`) and run in the `R` console:

```r
source("setup.R")
```

After running this command once, the project environment will load automatically whenever you open the project, for both project root and individual workshops. 
...
We also provide a Docker image which can be used to create an RStudio container with the required packages installed. 

To do so, after cloning the git repository, run from the project root:

```bash
docker pull aasohail/bayescog_test:latest

docker run -p 8787:8787 \
  --mount type=bind,source=$(pwd),target=/home/rstudio/project \
  -e PASSWORD=your_chosen_password \
  --name bayescog_test \
  aasohail/bayescog_test
```
